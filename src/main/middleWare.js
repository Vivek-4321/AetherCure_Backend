...
export const verifyOtp = (secret, otp) => {
  try {
    ...
    // Use a much larger window to account for significant time drift
    // Increase from 2 to 10 (covers a wider time range)
    ...
    // For debugging purposes, also check direct string comparison
    const generatedToken = totp.generate();
    console.log("Current generated token:", generatedToken);
    const directMatch = generatedToken === otp.toString();
    console.log("Direct string match:", directMatch);

    // Return true if either validation method works
    return delta !== null || directMatch;
  } catch (error) {
...
export const validatePassword = async (password, storedHash) => {
  try {
    ...
    const hashBuffer = Uint8Array.from(atob(storedHash), (c) =>
      c.charCodeAt(0)
    );
